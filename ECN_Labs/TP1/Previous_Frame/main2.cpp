/*  Gosez Alexis - Rolin Aubin
    ...
    The previous frame of the video is the reference
*/
...
int main(int argc, char** argv )
{
    ...
    detector.detect( previous, keypoints_1 );
    ...
    keypoints_filtered.empty();

    extractor.compute( previous, keypoints_1, descriptors_1 );

    rectangle(previous, Point2f(24, 44), Point2f(196, 210), Scalar(0,0,255));
    ...
    for(;;)
    {
        ...
        //The rotation angle between the two images
        double angle = atan2(outputArray[1].y-outputArray[0].y, outputArray[1].x-outputArray[0].x);
        double norme = norm(outputArray[1]-outputArray[0]);

        //To draw the rotated rectangle
        outputArray[3].x = outputArray[0].x + norme*sin(angle);
        outputArray[3].y = outputArray[0].y - norme*cos(angle);

        outputArray[2].x = outputArray[1].x + norme*sin(angle);
        outputArray[2].y = outputArray[1].y - norme*cos(angle);
        ...
        Mat img_matches;
        drawMatches( previous, keypoints_1, gray, keypoints_2,
                   good_matches, img_matches, Scalar::all(-1), Scalar::all(-1),
                   vector<char>(), DrawMatchesFlags::NOT_DRAW_SINGLE_POINTS );

        //Show detected matches.copy()
        ...
        //We only keep the good keypoints
        for (int i = 0 ; i < keypoints_2.size() ; i++)
        {
            if (norm(keypoints_2[i].pt - Point2f(((outputArray[0].x+outputArray[2].x)/2),
                                                    (outputArray[0].y+outputArray[2].y)/2)) < 0.5*norm(outputArray[0]-outputArray[1]))
            {
                keypoints_filtered.push_back(keypoints_2[i]);
            }
        }

        keypoints_2 = keypoints_filtered;
